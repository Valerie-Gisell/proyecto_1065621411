...
import { getMonthlyBalance } from "@/lib/dataService";
...
export async function GET(request: Request) {
  ...
  const balance = await getMonthlyBalance(auth.sub, year, month);
  return NextResponse.json({ success: true, data: balance, timestamp: new Date().toISOString() });
}
