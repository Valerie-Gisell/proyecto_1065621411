...
import { getCategoryBreakdown } from "@/lib/dataService";
...
export async function GET(request: Request) {
  ...
  const breakdown = await getCategoryBreakdown(auth.sub, year, month);
  return NextResponse.json({ success: true, data: breakdown, timestamp: new Date().toISOString() });
}
